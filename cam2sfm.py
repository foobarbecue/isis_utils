...
def cam_csv(ground_points, to_file):
    ...
    for ground_point in ground_points:
        ...
        sun_angles = [ground_point['Incidence'].value, ground_point['SubSolarAzimuth'].value]
        df.loc[name, :] = pos + rot + pointing + look + [alt] + sun_direction_vector.tolist() + sun_angles
    df.to_csv(to_file)
    return
